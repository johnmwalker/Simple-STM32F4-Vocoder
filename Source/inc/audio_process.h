...
void echo(int16_t* buffer, uint32_t size,  int16_t ms);
void sineCarrier(int16_t* buffer, uint32_t size, int16_t freq);
void squareCarrier(int16_t* buffer, uint32_t size, int16_t freq);
void triCarrier(int16_t* buffer, uint32_t size, int16_t freq);
void vibrato(int16_t* buffer, uint32_t size, int16_t freq);

//Effect presets
void vader(int16_t* buffer, uint32_t size);
void probeDroid(int16_t* buffer, uint32_t size);
void swoosh(int16_t* buffer, uint32_t size);
...
void sineCarrier(int16_t* buffer, uint32_t size, int16_t freq)
{
	...
	for(i=0; i<size; i++)
	{
		...
		float temp = (sinf(TWOPI*i/16000.0*freq)) * (buffer[i]-offset);
		buffer[i] = temp+offset;
	}
}
...
void squareCarrier(int16_t* buffer, uint32_t size, int16_t freq)
{
	...
	{	if(j<period/2)
		...
		j++;
		//The 1800 in this case is approximately the 1.5V offset on the input signal
		float temp = val * (buffer[i]-offset);
		buffer[i] = temp+offset;
	}
}
